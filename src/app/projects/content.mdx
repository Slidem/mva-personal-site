import Layout from "@/components/ProjectsLayout";
...
### LinkedIn Integration and Chrome Extension Development:
- **Reduced regressions by 50%** by setting up end to end tests leveraging playwright framework & checkly and creating proper dashboards and alerts in CloudWatch.
- **Doubled load time** of the frontend application by eliminating unnecessary rendering of React components, removing improper uses of ReactContext, and adding lazy rendering and pagination where needed.
- **Improved AWS the serverless architecture** of the application, by splitting large lambdas into smaller, reausable isolated ones; improving isolation of staging & production environments (wrongly using the same buckets for email sending).
